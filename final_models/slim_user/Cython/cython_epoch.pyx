...
from utils import similarity_matrix_topK, check_matrix
import numpy as np
cimport numpy as np
import time
import sys
...
cdef struct Sample:
    long track_id
    long positive_playlist_id
    long negative_playlist_id


cdef class SLIM_BPR_User_Cython_Epoch:
    ...
    cdef long[:] eligible_tracks
    cdef long n_eligible_tracks

    cdef set target_playlists
    cdef set target_tracks

    cdef int[:] sampled_track_playlists
    cdef int n_sampled_track_playlists
    ...
    def __init__(self, URM_mask, eligible_tracks, target_tracks, target_playlists, sgd_mode='adagrad'):
        super(SLIM_BPR_User_Cython_Epoch, self).__init__()

        URM_mask = check_matrix(URM_mask, 'csr')

        self.num_positive_iteractions = int(URM_mask.nnz * 1)
        self.n_tracks = URM_mask.shape[0]
        self.n_playlists = URM_mask.shape[1]
        ...
        if self.sparse_weights:
            self.S_sparse = Sparse_Matrix_Tree_CSR(self.n_playlists, self.n_playlists)
        else:
            self.S_dense = np.zeros((self.n_playlists, self.n_playlists), dtype=np.float64)
        ...
        if sgd_mode == 'adagrad':
            ...
            raise ValueError("SGD_mode not valid. Acceptable values are: 'sgd', 'adagrad', 'rmsprop'. Provided value was '{}'".format(sgd_mode))

        self.eligible_tracks = eligible_tracks
        self.n_eligible_tracks = len(eligible_tracks)
    ...
    cdef int[:] get_track_playlists(self, long index):
        return self.URM_mask_indices[self.URM_mask_indptr[index]: self.URM_mask_indptr[index + 1]]

    def epoch_iteration_cython(self, learning_rate=0.05, batch_size=10, topK=100, lambda_i=0.0025, lambda_j=0.00025):
        ...
        cdef long total_number_of_batch = int(self.num_positive_iteractions / self.batch_size) + 1

        cdef Sample sample
        cdef long i, j
        cdef long index, seenItem, current_batch
        cdef float x_uij, gradient

        cdef int numSeenItems
        cdef int print_step = 500000 if self.sparse_weights else 5000000

        # Variables for AdaGrad and RMSprop
        cdef double [:] sgd_cache
        cdef double cache_update
        cdef float gamma

        if self.useAdaGrad:
            sgd_cache = np.zeros(self.n_playlists, dtype=float)
        ...
        for current_batch in range(total_number_of_batch):
            sample = self.sample_batch_cython()

            i = sample.positive_playlist_id
            j = sample.negative_playlist_id

            x_uij = 0.0

            # The difference is computed on the user_seen items
            index = 0
            while index < self.n_sampled_track_playlists:
                seenItem = self.sampled_track_playlists[index]
                index += 1
            ...
            if self.useAdaGrad:
                cache_update = gradient ** 2

                sgd_cache[i] += cache_update
                sgd_cache[j] += cache_update

                gradient = gradient / (sqrt(sgd_cache[i]) + 1e-8)
            elif self.rmsprop:
                cache_update = sgd_cache[i] * gamma + (1 - gamma) * gradient ** 2
            ...
            while index < self.n_sampled_track_playlists:
                ...
                if self.sparse_weights:
                    if seenItem != i:
                        self.S_sparse.add_value(i, seenItem, (gradient - self.S_sparse.get_value(i, seenItem) * self.lambda_i) * self.learning_rate)
                    if seenItem != j:
                        self.S_sparse.add_value(j, seenItem, (-gradient - self.S_sparse.get_value(i, seenItem) * self.lambda_j) * self.learning_rate)
                else:
                    if seenItem != i:
            ...
            if current_batch % print_step == 0 and not current_batch == 0 or current_batch == total_number_of_batch - 1:
                print("Processed {} ( {:.2f}% ) in {:.2f} seconds. Sample per second: {:.0f}".format(
                    current_batch * self.batch_size,
                    100.0 * float(current_batch * self.batch_size) / self.num_positive_iteractions,
                    time.time() - start_time_batch,
                    float(current_batch * self.batch_size + 1) / (time.time() - start_time_epoch)))
        ...
        if self.sparse_weights:
            ...
            return similarity_matrix_topK(np.array(self.S_dense.T), k=self.topK, forceSparseOutput=True, inplace=True).T


    cdef Sample sample_batch_cython(self):
        ...
        index = int(rand() / RAND_MAX_DOUBLE * self.n_eligible_tracks)

        sample.track_id = self.eligible_tracks[index]

        self.sampled_track_playlists = self.get_track_playlists(sample.track_id)
        self.n_sampled_track_playlists = len(self.sampled_track_playlists)

        index = int(rand() / RAND_MAX_DOUBLE * self.n_sampled_track_playlists)

        sample.positive_playlist_id = self.sampled_track_playlists[index]
        ...
        while not negative_track_selected:
            sample.negative_playlist_id = int(rand() / RAND_MAX_DOUBLE  * self.n_playlists)

            index = 0
            while index < self.n_sampled_track_playlists and self.sampled_track_playlists[index] != sample.negative_playlist_id:
                index += 1

            if index == self.n_sampled_track_playlists and (sample.track_id not in self.target_tracks or sample.negative_playlist_id not in self.target_playlists):
