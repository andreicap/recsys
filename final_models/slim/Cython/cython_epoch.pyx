...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    def __init__(self, URM_mask, sparse_weights, eligible_playlists, target_tracks, target_playlists, learning_rate=0.05,
                 batch_size=10, topK=100, sgd_mode='adagrad', lambda_i=0.0025, lambda_j=0.00025):
        ...
        if self.sparse_weights:
            self.S_sparse = Sparse_Matrix_Tree_CSR(self.n_tracks, self.n_tracks)
        else:
            self.S_dense = np.zeros((self.n_tracks, self.n_tracks), dtype=np.float64)
    ...
    def epoch_iteration_cython(self):
        ...
        if self.useAdaGrad:
            sgd_cache = np.zeros(self.n_tracks, dtype=float)
        ...
        for current_batch in range(total_number_of_batch):
            ...
            index = 0
            while index < self.n_sampled_playlist_tracks:
                seenItem = self.sampled_playlist_tracks[index]
                index += 1
    ...
    cdef Sample sample_batch_cython(self):
        ...
        while not negative_track_selected:
            ...
            if index == self.n_sampled_playlist_tracks and (sample.playlist_id not in self.target_playlists or sample.negative_track_id not in self.target_tracks):
                # index1 = 0
                # while index1 < self.n_target_tracks and self.target_tracks[index1] != sample.negative_track_id:
                #     index1 += 1
                # if index1 == self.n_target_tracks:
